...
namespace WebServer.ByTheCakeApplication.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            if (!req.Session.Contains(SessionStorage.CurrentUserKey))
            {
                throw new InvalidOperationException("There is no logged in user.");
            }

            var username = req.Session.Get<string>(SessionStorage.CurrentUserKey);
        ...
        {
            req.Session.Add(SessionStorage.CurrentUserKey, username);
            req.Session.Add(SessionStorage.SessionKey, new ShoppingCart());
        }
    }
}
